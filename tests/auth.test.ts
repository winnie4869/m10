...
import { ACCESS_TOKEN_COOKIE_NAME, REFRESH_TOKEN_COOKIE_NAME } from '../src/lib/constants';
...
describe('인증 API (로그인/회원가입)', () => {
  ...
  describe('회원가입 (POST /auth/register)', () => {
    it('성공: 새로운 사용자를 생성하고 201 상태 코드를 반환해야 합니다.', async () => {
    ...
    it('실패: 이미 존재하는 이메일로 가입 시 400 에러를 반환해야 합니다.', async () => {
      const response = await request(app)
        .post('/auth/register')
        .send({
          ...testUser,
          nickname: 'another-nickname', // 닉네임은 달라도 이메일이 같으면 실패해야 함
        });
  ...
  describe('로그인 (POST /auth/login)', () => {
    it('성공: 올바른 이메일과 비밀번호로 로그인 시 200 상태 코드와 토큰 쿠키를 반환해야 합니다.', async () => {
      const response = await request(app)
        .post('/auth/login')
        .send({
    ...
    it('실패: 잘못된 비밀번호로 로그인 시 400 에러를 반환해야 합니다.', async () => {
      const response = await request(app)
        .post('/auth/login')
        .send({
          email: testUser.email,
    ...
    it('실패: 존재하지 않는 이메일로 로그인 시 400 에러를 반환해야 합니다.', async () => {
      const response = await request(app)
        .post('/auth/login')
        .send({
